"""Layer-wise Representation Distance analysis for Llama-3.2-1B.
...
FINETUNED_PATHS = {
    "Sentiment":      "<DATA_ROOT>/fine_tuned_models/llama3.2-yelp",
    "Sentiment_SST2": "<DATA_ROOT>/fine_tuned_models/llama3.2-sst2",
    "QA":             "<DATA_ROOT>/fine_tuned_models/llama3.2-squad",
    "QA_CoQA":        "<DATA_ROOT>/fine_tuned_models/llama3.2-coqa",
    "MT":             "<DATA_ROOT>/fine_tuned_models/llama3.2-kde4",
    "MT_tatoeba":     "<DATA_ROOT>/fine_tuned_models/llama3.2-tatoeba",
}

OUTPUT_DIR = os.environ.get(
    "OUTPUT_DIR",
    f"./Results/{CURRENT_TASK}/Llama3_hfdir/{datetime.now().strftime('%Y%m%d_%H%M%S')}",
)
...
def load_tl_model(ft_path=None):
    print(f"Loading Llama-3.2 (ft dir: {ft_path})...")
    if ft_path is None:
