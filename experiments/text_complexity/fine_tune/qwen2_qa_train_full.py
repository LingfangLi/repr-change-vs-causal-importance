...
Hyperparameters mirror src/Fine_tune/Question_answering/Qwen2-0.5B-7b-squad-full.py.
"""
import os
import json
...
config = {
    ...
    # Training params (mirror Qwen2-0.5B-7b-squad-full.py)
    "max_seq_length": 1024,
    "learning_rate": 2e-5,
    "batch_size": 4,
...
run_name = f"qwen2-0.5b-squad-{EXPERIMENT_TYPE}-full-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
output_dir = os.path.join(OUTPUT_BASE_DIR, run_name)
os.environ.setdefault("WANDB_PROJECT", "MI_qwen2-squad-text_complexity-full")
...
def main():
    ...
    print(f"Starting Qwen2-0.5B squad-{EXPERIMENT_TYPE} full FT: {run_name}")
    trainer.train()
